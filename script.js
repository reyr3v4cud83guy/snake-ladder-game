document.addEventListener("DOMContentLoaded", () => {
  ...
  diceThrow.addEventListener("click", () => {
    const diceValue = Math.floor(Math.random() * 6) + 1;
    diceThrow.textContent = diceValue;
    message.textContent = `${currentPlayer} rolled a ${diceValue}`;
    // ...existing code to handle game logic...
  });

  resetButton.addEventListener("click", () => {
    // ...existing code to reset the game...
  });

  // ...existing code to draw the game board and handle game logic...
});
...
const checkObstacles = (player) => {
  for (let i = 0; i < obstacles.length; i++) {
    if (obstacles[i].start === player.target) {
      let endSquare = obstacles[i].end;
      player.target = obstacles[i].end;
      slide(
        player,
        walkSequence[endSquare - 1].x,
        walkSequence[endSquare - 1].y,
